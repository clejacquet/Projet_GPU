...
static const char *kernel_name = "compute";

bool* tiles_changed;
unsigned tiles_xsize, tiles_ysize;
...
cl_mem tex_buffer, cur_buffer, next_buffer, tile_cur_buffer, tile_next_buffer;
...
void ocl_init(void) {
    ...
    if (str != NULL) {
        kernel_name = str;
        printf("ENV: %s\n", kernel_name);
    } else if (opencl_kernel_names[version] != NULL) {
        kernel_name = opencl_kernel_names[version];
        printf("FOUND: %s\n", kernel_name);
    } else {
        printf("NOT FOUND: %s\n", kernel_name);
    }
    ...
    tiles_xsize = (unsigned) ceil(1.0 * DIM / TILEX);
    tiles_ysize = (unsigned) ceil(1.0 * DIM / TILEY);
    int tiles_changed_size = tiles_xsize * tiles_ysize;
    tiles_changed = malloc(tiles_changed_size * sizeof(bool));

    for (int i = 0;i < tiles_changed_size; i++) {
        tiles_changed[i] = 1;
    }
    ...
    //
    {
        ...
        sprintf(flags,
                "-cl-mad-enable -cl-fast-relaxed-math -DDIM=%d -DSIZE=%d -DTILEX=%d -DTILEY=%d -DTILECX=%d -DTILECY=%d",
                DIM, SIZE, TILEX, TILEY, tiles_xsize, tiles_ysize);
    ...
    if (opencl_tiles_used[version]) {
        tile_cur_buffer = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(unsigned) * tiles_xsize * tiles_ysize,
                                         NULL, NULL);

        printf("Tiled with OpenCL used\n");
        if (!tile_cur_buffer)
            exit_with_error ("Failed to allocate tile input buffer");

        tile_next_buffer = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(unsigned) * tiles_xsize * tiles_ysize,
                                          NULL, NULL);
        if (!tile_next_buffer)
            exit_with_error ("Failed to allocate tile output buffer");
    }


    printf("Using %dx%d workitems grouped in %dx%d tiles [total of %dx%d]\n", SIZE, SIZE, TILEX, TILEY,
            tiles_xsize, tiles_ysize);
}
...
void ocl_send_image(unsigned *image) {
    ...
    if (opencl_tiles_used[version]) {
        err = clEnqueueWriteBuffer(queue, tile_cur_buffer, CL_TRUE, 0,
                                   sizeof(unsigned) * tiles_xsize * tiles_ysize, tiles_changed, 0, NULL, NULL);
        check (err, "Failed to write to cur_buffer");

        err = clEnqueueWriteBuffer(queue, tile_next_buffer, CL_TRUE, 0,
                                   sizeof(unsigned) * tiles_xsize * tiles_ysize, tiles_changed, 0, NULL, NULL);
        check (err, "Failed to write to next_buffer");
    }
...
unsigned ocl_compute_with_tiles(unsigned nb_iter) {
    ...
    for (unsigned it = 1; it <= nb_iter; it++) {
        ...
        err = clSetKernelArg(compute_kernel, 0, sizeof(cl_mem), &cur_buffer);
        err = clSetKernelArg(compute_kernel, 1, sizeof(cl_mem), &next_buffer);
        err = clSetKernelArg(compute_kernel, 2, sizeof(cl_mem), &tile_cur_buffer);
        err = clSetKernelArg(compute_kernel, 3, sizeof(cl_mem), &tile_next_buffer);
        check (err, "Failed to set kernel arguments");
        ...
        // Swap buffers
        {
            ...
            tmp = tile_cur_buffer;
            tile_cur_buffer = tile_next_buffer;
            tile_next_buffer = tmp;
        }
    }
